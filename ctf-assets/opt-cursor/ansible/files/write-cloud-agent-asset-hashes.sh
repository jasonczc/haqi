...
manifest_path="${1:-/packages/agent-controller/assets/cloud-agent-assets-manifest.json}"
asset_root="${2:-/packages/agent-controller/assets}"
...
if [ -n "$asset_rows" ]; then
    while IFS=$'\t' read -r source_path destination_path; do
        ...
        if [ ! -f "$source_file" ]; then
            echo "ERROR: cloud agent source asset missing at ${source_file}" >&2
            exit 1
        fi

        if [ ! -f "$destination_path" ]; then
            echo "ERROR: cloud agent destination asset missing at ${destination_path}" >&2
            exit 1
        fi
