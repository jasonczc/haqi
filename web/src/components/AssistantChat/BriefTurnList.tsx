...
function LivePreviewCarousel(props: {
    ...
    return (
        <div className="min-h-[3.6rem] py-0.5">
            <div className={`whitespace-pre-wrap break-words text-sm leading-6 text-[var(--cursor-text-tertiary)] transition-all duration-200 ease-out ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-1 opacity-0'}`}>
                {activeLine}
            </div>
            <div className="truncate text-xs leading-5 text-[var(--cursor-text-tertiary)]">
                {props.liveActivity}
            </div>
        </div>
    )
            ...
            <div className="space-y-2">
                ...
                <div className="flex justify-start">
                    {isLiveTurn ? (
                        ...
                        <div className="w-full max-w-[92%] px-1 py-1">
                            <BriefFullMarkdownContent content={assistantPreview} />
                            <div className="mt-2 flex items-center gap-2 text-[11px] text-[var(--cursor-text-tertiary)]">
                                <span>{messageMeta}</span>
                                <span>·</span>
                                <button
                                    type="button"
                    ...
                    <div className="min-h-0 flex flex-1 flex-col pr-1">
                        ...
                        >
                            {props.isLoading && props.turns.length === 0 ? (
                                <div className="text-xs text-[var(--cursor-text-tertiary)]">Loading conversation…</div>
                            ) : null}

                            {props.turns.length === 0 && !props.isLoading ? (
                                <div className="text-xs text-[var(--cursor-text-tertiary)]">No turns yet.</div>
                            ) : null}
                    ...
                    <div className="fixed inset-0 z-[60] flex flex-col bg-[var(--cursor-bg-card)]">
                        <div className="border-b border-[var(--cursor-stroke-primary)] bg-[var(--cursor-bg-card)] px-3 py-2 pt-[calc(0.5rem+env(safe-area-inset-top))]">
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-[var(--cursor-stroke-primary)] text-[var(--cursor-text-tertiary)] transition-colors hover:bg-[var(--cursor-bg-quiet)] hover:text-[var(--cursor-text-primary)]"
                                    aria-label="Back"
                                >
                                    <BackIcon />
                                </button>
                        ...
                        <div className="min-h-0 flex-1 overflow-hidden pb-[env(safe-area-inset-bottom)]">
                            ...
                            {activeTurn && activeDetail && !activeDetail.error ? (
                                ...
                                <div className="flex h-full items-center justify-center text-sm text-[var(--cursor-text-tertiary)]">
                    ...
                    <DialogContent className="flex h-[90vh] max-h-[90vh] max-w-4xl flex-col overflow-hidden p-0">
                        ...
                        <div className="min-h-0 flex-1 overflow-hidden">
                            ...
                            {activeTurn && activeDetail && !activeDetail.error ? (
                                ...
                                <div className="flex h-[65vh] items-center justify-center text-sm text-[var(--cursor-text-tertiary)]">
                                    {activeTurn && activeDetail?.isLoading ? (
                                        <span className="inline-flex items-center gap-2">
                                            <Spinner size="sm" label={null} className="text-current" />
                                            Loading turn details…
