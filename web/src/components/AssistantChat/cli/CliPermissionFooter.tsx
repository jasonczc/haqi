...
function CliBtn(props: {
    ...
    const color = props.tone === 'allow'
        ...
        : props.tone === 'deny'
            ? 'text-[var(--danger)] border-[var(--danger)]/40 hover:bg-[var(--danger)]/10'
            : 'text-[var(--cursor-link)] border-[var(--cursor-stroke-primary)] hover:bg-[var(--cursor-bg-quiet)]'
...
export const CliPermission = memo(function CliPermission(props: {
    ...
    if (permission.status !== 'pending') {
        if ((permission.status === 'denied' || permission.status === 'canceled') && permission.reason) {
            return <div className="ml-5 text-xs italic text-[var(--danger)]">denied: {permission.reason}</div>
        }
        return null
    }
...
export const CliAskUserQuestion = memo(function CliAskUserQuestion(props: {
    ...
    return (
        <div className="ml-5 mt-1 border-l border-[var(--cursor-stroke-secondary)] pl-3 space-y-1">
            {questions.length === 0 ? (
                <div className="space-y-1">
                    <span className="text-xs text-[var(--cursor-info)]">? Input requested</span>
                    <input
                        type="text"
                        value={fallbackText}
                        ...
                        className="block w-full max-w-md rounded border border-[var(--cursor-stroke-primary)] bg-transparent px-2 py-1 text-xs text-[var(--cursor-text-primary)] placeholder:text-[var(--cursor-text-secondary)] focus:outline-none focus:border-[var(--cursor-link)]"
                    />
                </div>
            ) : (
                    ...
                    <div key={qIdx} className="space-y-0.5">
                                ...
                                return (
                                    <button
                                        ...
                                        className={`inline-flex items-center rounded border px-2 py-0.5 text-xs transition-colors disabled:opacity-40 ${selected
                                            ...
                                            : 'border-[var(--cursor-stroke-primary)] text-[var(--cursor-text-secondary)] hover:text-[var(--cursor-text-primary)] hover:bg-[var(--cursor-bg-quiet)]'
                                        }`}
                        ...
                        {otherSelectedByQ[qIdx] && (
                            <input
                                ...
                                disabled={props.disabled || loading}
                                placeholder="Type your answer…"
                                className="mt-0.5 block w-full max-w-md rounded border border-[var(--cursor-stroke-primary)] bg-transparent px-2 py-1 text-xs text-[var(--cursor-text-primary)] placeholder:text-[var(--cursor-text-secondary)] focus:outline-none focus:border-[var(--cursor-link)]"
                            />
                        )}
                    </div>
                ))
