...
function ResultPanel(props: {
    ...
    return (
        <div className="ml-5 mt-0.5 border-l border-[var(--cursor-stroke-secondary)] pl-3 text-xs text-[var(--cursor-text-secondary)]">
            ...
            {isError && (
                <div className="text-[var(--cursor-danger)] mt-0.5">
                    error
                </div>
            )}
        </div>
...
export const CliToolBlock = memo(function CliToolBlock(props: { block: ToolCallBlock }) {
    ...
    const stateColor = isError
        ...
        : isPending
            ? 'text-[var(--cursor-warning)]'
            : isRunning
                ? 'text-[var(--cursor-text-secondary)]'
                : 'text-[var(--cursor-text-secondary)] opacity-60'
    ...
    return (
        <div className="py-0.5">
            ...
            <button
                ...
                className={`flex items-center gap-1.5 text-xs rounded px-1 -mx-1 w-full text-left transition-colors ${hasResult ? 'hover:bg-[var(--cursor-bg-quiet)] cursor-pointer' : 'cursor-default'}`}
            >
                {/* Status indicator */}
                ...
                {/* Title */}
                <span className={`shrink-0 ${isRunning ? 'text-[var(--cursor-text-primary)]' : 'text-[var(--cursor-text-secondary)]'}`}>
                    {pres.title}
                </span>

                {/* Subtitle / key param */}
                {pres.subtitle && (
                    <span className="text-[var(--cursor-text-secondary)] truncate opacity-70">
                        {pres.subtitle}
                    </span>
                )}
                ...
                {block.tool.completedAt && block.tool.startedAt && (
                    <span className="ml-auto shrink-0 text-[var(--cursor-text-secondary)] opacity-50">
            ...
            {block.children.length > 0 && (
                <div className="ml-5 border-l border-[var(--cursor-stroke-secondary)] pl-2">
            ...
            {isAskUserQuestionToolName(block.tool.name) && block.tool.permission?.status === 'pending' ? (
                ...
                <div className="ml-5 text-xs italic text-[var(--danger)]">denied: {block.tool.permission.reason}</div>
            ) : null}
        </div>
    )
})
