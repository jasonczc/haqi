...
function OfflineSectionRow(props: {
    ...
    label?: string
    onToggleGroup: (directory: string, isCollapsed: boolean) => void
    onBringAllOnline?: () => void
}) {
    const { t } = useTranslation()
    const { directory, count, isCollapsed, density, label, onToggleGroup, onBringAllOnline } = props
    const [menuOpen, setMenuOpen] = useState(false)
    const [menuAnchorPoint, setMenuAnchorPoint] = useState<{ x: number; y: number }>({ x: 0, y: 0 })
    ...
    return (
        <>
            <button
                ...
                onContextMenu={onBringAllOnline ? (event) => {
                    event.preventDefault()
                    event.stopPropagation()
                    setMenuAnchorPoint({ x: event.clientX, y: event.clientY })
                    setMenuOpen(true)
            ...
            {menuOpen && onBringAllOnline ? (
                <div
                    ref={menuRef}
                    role="menu"
                    aria-label={t('sessions.projectOffline.section')}
                    className="fixed z-50 min-w-[220px] rounded-lg border border-[var(--app-border)] bg-[var(--app-bg)] p-1 shadow-lg animate-menu-pop"
                    style={{ top: menuAnchorPoint.y + 4, left: menuAnchorPoint.x }}
                >
                    <button
                        type="button"
                        role="menuitem"
                    ...
                    >
                        <BringOnlineIcon className="text-[var(--app-hint)]" />
                        {t('sessions.projectOffline.bringAllOnline')}
                    </button>
                </div>
            ) : null}
        </>
...
function BringOnlineIcon(props: { className?: string }) {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={props.className} aria-hidden="true">
                                ...
                                if (row.type === 'projects-offline-section') {
                                    return (
                                        <OfflineSectionRow
                                            ...
                                            onBringAllOnline={() => setProjectOfflineDirectories(new Set())}
                                        />
                                    )
                                }
