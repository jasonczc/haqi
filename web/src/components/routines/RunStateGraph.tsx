...
// Node palette — sources from cursor-theme.css tokens via var() refs
// so the graph follows theme changes (incl. any future dark variant).
// Each state picks one of the semantic buckets (success/warn/danger/
// accent) or falls back to the neutral surface tokens.
const STATE_COLORS: Record<
    RoutineRunStatus,
    { bg: string; border: string; text: string }
> = {
    queued:    { bg: 'var(--bg-tertiary)',        border: 'var(--border-secondary)', text: 'var(--text-secondary)' },
    spawning:  { bg: 'var(--bg-warn-tertiary)',   border: 'var(--warn)',             text: 'var(--warn)' },
    running:   { bg: 'var(--bg-accent-tertiary)', border: 'var(--accent)',           text: 'var(--accent)' },
    succeeded: { bg: 'var(--bg-success-tertiary)',border: 'var(--success)',          text: 'var(--success)' },
    failed:    { bg: 'var(--bg-danger-tertiary)', border: 'var(--danger)',           text: 'var(--danger)' },
    timeout:   { bg: 'var(--bg-warn-tertiary)',   border: 'var(--warn)',             text: 'var(--warn)' },
    cancelled: { bg: 'var(--bg-secondary)',       border: 'var(--border-secondary)', text: 'var(--text-secondary)' },
    skipped:   { bg: 'var(--bg-quaternary)',      border: 'var(--border-secondary)', text: 'var(--text-tertiary)' }
}
...
function StateNode({ data }: NodeProps<NodeData>) {
    ...
    const ring = data.isCurrent
        ? '0 0 0 2px var(--text-primary), 0 0 12px 4px var(--shadow-primary)'
        : 'none'
    return (
        <div
            style={{
        ...
        for (const [from, toList] of Object.entries(TRANSITIONS) as [RoutineRunStatus, RoutineRunStatus[]][]) {
            for (const to of toList) {
                ...
                e.push({
                    ...
                    target: to,
                    style: {
                        stroke: active ? 'var(--text-primary)' : 'var(--border-primary)',
                        strokeWidth: active ? 2 : 1,
                        opacity: active ? 1 : 0.45
                    },
                    markerEnd: {
                        type: 'arrowclosed' as const,
                        color: active ? 'var(--text-primary)' : 'var(--border-primary)',
                        width: 14,
                        height: 14
                    } as any,
