...
function RawJsonDevOnly(props: { value: unknown }) {
    ...
    return (
        <details className="mt-3">
            <summary className="cursor-pointer text-xs font-medium text-[var(--cursor-text-tertiary)]">
                Raw JSON
            </summary>
            <div className="mt-2">
...
const BashResultView: ToolViewComponent = (props: ToolViewProps) => {
    ...
    if (result === undefined || result === null) {
        return <div className="text-sm text-[var(--cursor-text-tertiary)]">{placeholderForState(props.block.tool.state)}</div>
    }
    ...
    return (
        <>
            <div className="text-sm text-[var(--cursor-text-tertiary)]">(no output)</div>
            <RawJsonDevOnly value={result} />
        </>
    )
}
...
const ReadResultView: ToolViewComponent = (props: ToolViewProps) => {
    ...
    if (file) {
        ...
        return (
            <>
                {path ? (
                    <div className="mb-2 text-xs text-[var(--cursor-text-tertiary)] font-mono break-all">
                        {basename(path)}
                    </div>
                ) : null}
...
const MutationResultView: ToolViewComponent = (props: ToolViewProps) => {
    ...
    if (result === undefined || result === null) {
        if (state === 'completed') {
            return <div className="text-sm text-[var(--cursor-text-tertiary)]">Done</div>
        }
        return <div className="text-sm text-[var(--cursor-text-tertiary)]">{placeholderForState(state)}</div>
    }
    ...
    return (
        <>
            <div className="text-sm text-[var(--cursor-text-tertiary)]">
...
const CodexPatchResultView: ToolViewComponent = (props: ToolViewProps) => {
    ...
    if (result === undefined || result === null) {
        return props.block.tool.state === 'completed'
            ? <div className="text-sm text-[var(--cursor-text-tertiary)]">Done</div>
            : <div className="text-sm text-[var(--cursor-text-tertiary)]">{placeholderForState(props.block.tool.state)}</div>
    }
...
function collabStatusTone(status: string | null): string {
    if (!status) return 'text-[var(--cursor-text-tertiary)]'
...
const CollabAgentResultView: ToolViewComponent = (props: ToolViewProps) => {
    ...
    return (
        <div className="flex flex-col gap-2">
                    ...
                    <button
                        ...
                        className={`rounded border px-2 py-1 text-xs transition-colors ${
                            selected?.agentId === entry.agentId
                                ...
                                : 'border-[var(--cursor-stroke-primary)] text-[var(--cursor-text-tertiary)] hover:bg-[var(--cursor-bg-quiet)]'
                        }`}
            ...
            {selected ? (
                <div className="rounded-md border border-[var(--cursor-stroke-primary)] bg-[var(--cursor-bg-hover)] p-2 text-xs">
            ...
            {collab.prompt ? (
                <div className="rounded-md border border-[var(--cursor-stroke-primary)] p-2 text-xs text-[var(--cursor-text-tertiary)]">
...
const ImageViewResultView: ToolViewComponent = (props: ToolViewProps) => {
    ...
    return (
        <div className="flex flex-col gap-2">
            ...
            {previewUrl ? (
                <div className="overflow-hidden rounded border border-[var(--cursor-stroke-primary)] bg-[var(--cursor-bg-hover)] p-1">
                    <img
                        src={previewUrl}
                        alt={altText}
                ...
                <div className="text-sm text-[var(--cursor-text-tertiary)]">Loading image…</div>
            ) : errorMessage ? (
...
const GenericResultView: ToolViewComponent = (props: ToolViewProps) => {
    ...
    if (typeof result === 'string') {
        ...
        if (parsed) {
            return (
                <>
                    <div className="text-xs text-[var(--cursor-text-tertiary)] mb-2">
