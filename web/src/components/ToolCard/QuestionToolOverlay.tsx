...
function SelectionMark(props: { checked: boolean; mode: 'single' | 'multi' }) {
    const mark = props.mode === 'multi'
        ...
        : (props.checked ? '●' : '○')
    return (
        <span className="mt-0.5 w-4 shrink-0 text-center text-[var(--cursor-text-tertiary)]">
            {mark}
        </span>
    )
}
...
function OptionRow(props: {
    ...
    return (
        <button
            type="button"
            className={cn(
                'flex w-full items-start gap-2 rounded-md px-2 py-2 text-left text-sm transition-colors hover:bg-[var(--cursor-bg-hover)] disabled:pointer-events-none disabled:opacity-50',
                props.checked ? 'bg-[var(--cursor-bg-hover)]' : null
            )}
    ...
    return (
        <div className="fixed inset-0 z-[70]">
            ...
            <div className="absolute inset-0 flex items-center justify-center p-3 sm:p-6">
                <div className="flex w-full max-w-2xl max-h-[calc(100dvh-env(safe-area-inset-top)-env(safe-area-inset-bottom)-1.5rem)] flex-col rounded-[10px] border border-[var(--cursor-stroke-primary)] bg-[var(--cursor-bg-quiet)] shadow-[0_4px_20px_rgba(0,0,0,0.08),0_1px_4px_rgba(0,0,0,0.04)]">
                    <div className="shrink-0 border-b border-[var(--cursor-stroke-primary)] px-4 py-3">
                        <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                                <div className="flex items-center gap-2">
                                    <Badge variant="default">{t('tool.questionOverlay.title')}</Badge>
                    ...
                    <div className="shrink-0 flex items-center justify-between gap-2 border-t border-[var(--cursor-stroke-primary)] px-4 py-3">
