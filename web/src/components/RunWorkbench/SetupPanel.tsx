...
function StepIcon(props: { status: string }) {
    if (props.status === 'done') {
        return (
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                <circle cx="9" cy="9" r="8" fill="var(--success)" />
    ...
    if (props.status === 'active') {
        return (
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" className="animate-spin">
                <circle cx="9" cy="9" r="7.5" stroke="var(--accent)" strokeWidth="1.5" strokeDasharray="35 12" />
            </svg>
        )
    }
    // pending / skipped
    return (
        <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
            <circle cx="9" cy="9" r="7.5" stroke="var(--border-secondary)" strokeWidth="1.5" />
        </svg>
    )
}
    ...
    return (
        <div className="flex flex-1 flex-col overflow-y-auto">
            ...
            {isSetupComplete && !checkpointId && (
                <div className="border-b border-[var(--border-tertiary)] bg-[var(--bg-quaternary)] px-4 py-3">
                        ...
                        <div key={i} className="flex items-start gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                                <div className={`text-[13px] font-medium ${
                                    step.status === 'done' ? 'text-[var(--text-primary)]' : 'text-[var(--text-tertiary)]'
                                }`}>
                                    {step.label}
                                </div>
            ...
            {isSetupComplete && containerId && (
                <div className="px-4 py-3">
                    ...
                    {/* Update Script */}
                    <div className="mb-3">
                        <div className="text-[12px] font-medium text-[var(--text-tertiary)] mb-1.5">
                            Update Script
                        </div>
                        <textarea
                            value={updateScript}
                            ...
                            rows={5}
                            className="w-full rounded-md border border-[var(--border-secondary)] bg-[var(--bg-quaternary)] px-3 py-2 font-mono text-[12px] text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)] resize-y"
                        />
                    </div>
                    ...
                    <div className="flex items-end gap-2">
                        <div className="flex-1">
                            <label className="text-[11px] font-medium text-[var(--text-tertiary)] mb-1 block">
                                Environment name
                            </label>
                            <input
                                type="text"
                                ...
                                className="w-full rounded-md border border-[var(--border-secondary)] bg-[var(--bg-editor)] px-3 py-1.5 text-[13px] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]"
                            />
                        </div>
                        <button
                            type="button"
                            ...
                            className="rounded-md bg-[var(--bg-neutral)] px-4 py-1.5 text-[13px] font-medium text-[var(--bg-editor)] transition-colors hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
            ...
            {checkpointId && (
                <div className="px-4 py-3">
                    <div className="flex items-center gap-2 rounded-md bg-[var(--bg-success-secondary)] px-3 py-2">
            ...
            {isSetupComplete && (
                <div className="px-4 py-3 border-t border-[var(--border-tertiary)]">
            ...
            {/* Environment info */}
            {environmentId && (
                <div className="border-t border-[var(--border-tertiary)] px-4 py-3">
