...
Namespaces are intended for small teams sharing a single public HAQI hub. Each team member uses a different namespace to isolate their sessions and machines without running separate hubs.
...
- One machine ID cannot be reused across namespaces.
  - To run multiple namespaces on one machine, use a separate `HAPI_HOME` per namespace, or clear the machine ID with `haqi auth logout` before switching.
